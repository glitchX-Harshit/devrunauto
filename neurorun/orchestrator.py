...
class NeuroOrchestrator:
    ...
    def plan_next_step(self, main_goal: str, current_image: Image.Image, step_count: int) -> Dict:
        ...
        If the keyboard is open and blocking the view, use "back" to close it.
        If the desired item (like 'Fries' image) is ALREADY visible on screen, prefer 'tap' over 'type'.
        ...
        {{
            ...
            "action": {{
                ...
                "bq_box": [ymin, xmin, ymax, xmax] (0-1000 scale) - REQUIRED for 'tap', OPTIONAL for 'type' (to tap before typing),
        ...
        for attempt in range(max_retries):
            try:
                # Add delay to respect rate limits (e.g. 10-15 RPM for free tier)
                if attempt == 0:
                    time.sleep(2) 
    ...
    async def execute_action_direct(self, action: Dict):
        ...
        if tipo == 'tap':
            ...
            # 1. Try Broadcast (ADB Keyboard) - Reliable for spaces
            text = action.get('text', '')
            
            # Escape for shell
            safe_text = text.replace("'", "'\\''")
            
            print(f"    -> Sending Broadcast Intent for text: {text}")
            os.system(f"adb -s {self.device_serial} shell am broadcast -a ADB_INPUT_TEXT --es msg '{safe_text}'")
            
            # 2. Fallback / Reinforce with standard input text (if broadcast fails or keyboard not set)
            # We wait a bit to see if it worked? No easy way. We just do both or standard if not spaces.
            # Ideally DroidRun setup installs 'ADB Keyboard'. If not present, this does nothing.
            
            # Let's DO standard input text as well for robustness, but separated.
            # Actually, standard input text is often safer if we clean spaces.
